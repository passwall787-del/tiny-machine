...
Godot 的 RigidBody2D 应通过力/冲量驱动，而不是在运行中持续直接修改位置。P1 的弹簧和齿轮使用一次性 `apply_central_impulse()` 触发。
...
Target 是 Area2D，使用 mask 检测 Ball。`body_entered` 用于接收 PhysicsBody2D 进入事件。
...
一次性冲量不能放在每帧循环中，否则会产生帧率相关行为；central impulse 只用于一次性冲击。
...
后续正式绳索应考虑 Joint2D，用物理关节绑定 PhysicsBody2D。
